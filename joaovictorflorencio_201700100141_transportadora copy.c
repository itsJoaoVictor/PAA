...
double calculateEfficiency(Package* p, int remaining_weight, int remaining_volume) {
    if (p->weight > remaining_weight || p->volume > remaining_volume) return -1;
...
int comparePackages(const void* a, const void* b) {
    ...
    return (p2->value / (p2->weight + p2->volume)) - (p1->value / (p1->weight + p1->volume));
}
...
void allocatePackages(Vehicle* vehicles, int n_vehicles, Package* packages, int m_packages) {
    Package** packagePointers = malloc(m_packages * sizeof(Package*));
    for (int i = 0; i < m_packages; i++) {
        packagePointers[i] = &packages[i];
    }
    
    qsort(packagePointers, m_packages, sizeof(Package*), comparePackages);
    ...
    for (int i = 0; i < n_vehicles; i++) {
        ...
        for (int j = 0; j < m_packages; j++) {
            Package* p = packagePointers[j];
            if (!p->used && p->weight <= remaining_weight && p->volume <= remaining_volume) {
                double efficiency = calculateEfficiency(p, remaining_weight, remaining_volume);
                if (efficiency > 0) {
